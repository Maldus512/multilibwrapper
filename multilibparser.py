...
def usage():
    print("usage: {} <path to shared library> <headers to parse for functions>".format(sys.argv[0]))
...
def main():
    ...
    with open("{}-wrapper.c".format(libname), "w") as f:
        f.write(final)
    ...
    with open("{}-wrapper.h".format(libname), "w") as f:
        f.write(final)
